...
public class AdminController {
    ...
    @GetMapping("/users")
    public ResponseEntity<Object> getAllUsers(@RequestParam(name = "ids", required = false) Long[] ids,
                                              @RequestParam(name = "from", defaultValue = "0") Integer from,
                                              @RequestParam(name = "size", defaultValue = "10") Integer size) {
        log.info("Получен запрос на получение списка пользователей с RequestParams : {},{},{}", ids, from, size);
        return ResponseEntity.status(200).body(userService.getAllUsers(ids, from, size));
    }
